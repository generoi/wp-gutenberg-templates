...
var GutenbergTemplates = function () {
  function GutenbergTemplates() {
    _classCallCheck(this, GutenbergTemplates);

    this.template = null;
    subscribe(this.subscribe.bind(this));
  }

  _createClass(GutenbergTemplates, [{
    key: 'subscribe',
    ...
    value: function changeTemplate() {
      ...
      var currentBlocks = select('core/editor').getBlocks();

      apiRequest({ path: '/gutenberg-templates/v1/template', data: { template: this.template } }).then(function (config) {
        resetBlocks(synchronizeBlocksWithTemplate(currentBlocks, config.template));
      });
    }
  }]);

  return GutenbergTemplates;
}();
